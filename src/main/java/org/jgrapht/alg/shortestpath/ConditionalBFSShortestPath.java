 ...
 */
public class ConditionalBFSShortestPath<V, E> extends BaseShortestPathAlgorithm<V, E> {
     ...
     */
    public ConditionalBFSShortestPath(Graph<V, E> graph, Condition<E> condition) {
        super(graph);
    ...
    public SingleSourcePaths<V, E> getPaths(V source) {
        ...
        while (!queue.isEmpty()) {
            V v = queue.poll();
            for (E e : graph.outgoingEdgesOf(v).stream().filter(e -> condition.test(e, distanceAndPredecessorMap.get(v).getFirst())).toList()) {
    ...
    public GraphPath<V, E> getPath(V source, V sink) {
        if (!graph.containsVertex(sink)) throw new IllegalArgumentException(GRAPH_MUST_CONTAIN_THE_SINK_VERTEX);
    ...
    public static <V, E> GraphPath<V, E> findPathBetween(Graph<V, E> graph, V source, V sink, Condition<E> condition) {
        return new ConditionalBFSShortestPath<>(graph, condition).getPath(source, sink);
    }
}